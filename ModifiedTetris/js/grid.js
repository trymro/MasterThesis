...
//adding option to remove random number of points from the grid
Grid.prototype.clearRandomPoints = function() {
    var randomPointsToClear = Math.floor(Math.random() * 220) + 1;

    for (var i = 0; i < randomPointsToClear; i++) {
        var randomRow = Math.floor(Math.random() * this.rows);
        var randomColumn = Math.floor(Math.random() * this.columns);
        this.cells[randomRow][randomColumn] = 0;
    }
};
...
//added function to count the filled lines
Grid.prototype.countFilledLines = function() {
